...
class KFACOptimizer(optim.Optimizer):
    ...
    def _update_inv(self, m, damping):
        ...
        if self.torch_symeig == 'true':
            ...
            self.H_a[m] = torch.linalg.inv(self.m_aa[m] + math.sqrt(damping) * torch.eye(self.m_aa[m].size(0)).to(self.m_aa[m].device))
            self.H_g[m] = torch.linalg.inv(self.m_gg[m] + math.sqrt(damping) * torch.eye(self.m_gg[m].size(0)).to(self.m_gg[m].device))

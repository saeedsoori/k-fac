...
class KBFGSLMEOptimizer(optim.Optimizer):
    def __init__(self,
                 model,
                 lr=0.001,
                 momentum=0.9,
        ...
        super(KBFGSLMEOptimizer, self).__init__(model.parameters(), defaults)
        self.handles = []
    ...
    def _get_LBFGS_Hv(self, m, v):
        ...
        p1 = torch.mm(self.R_inv[m].t(), torch.mm(torch.diag(self.D_diag[m]) + gamma * self.yTy[m], torch.mm(self.R_inv[m], torch.mm(self.s[m], v)))) -\
            gamma * torch.mm(self.R_inv[m].t(), torch.mm(self.y[m], v))
        p2 = -torch.mm(self.R_inv[m], torch.mm(self.s[m], v))

        Hv = gamma * v + torch.mm(self.s[m].t(), p1) + gamma * torch.mm(self.y[m].t(), p2)
    ...
    def _get_layer_update_direction(self, m, p_grad_mat, damping):
        ...
        """
        mat = torch.mm(p_grad_mat, self.H_a[m])

        v = self._get_LBFGS_Hv(m, mat[:, 0]).unsqueeze(1)
        for i in range(1, mat.size(1)):
            v = torch.cat([v, self._get_LBFGS_Hv(m, mat[:, i]).unsqueeze(1)], dim=1)

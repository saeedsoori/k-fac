...
class KBFGSL2LOOPOptimizer(optim.Optimizer):
    def __init__(self,
                 model,
                 lr=0.001,
                 momentum=0.9,
        ...
        super(KBFGSL2LOOPOptimizer, self).__init__(model.parameters(), defaults)
        self.handles = []
        ...
        self.s, self.y, self.rho, self.alpha = {}, {}, {}, {}
    ...
    def _get_LBFGS_Hv(self, m, v):
        if m not in self.s: # init Hg as I
            self.alpha[m] = [] # init
            return v
        ...
        # Algorithm 7.4 (L-BFGS two-loop recursion) in Numerical Optimization
        # Jorge Nocedal, Stephen J Wright 2006
        for c in range(v.size(1)):
            q = v[:, c]
            for i in range(len(self.rho[m]) - 1, -1, -1): # from the most recent update to the oldest update
                self.alpha[m].append(self.rho[m][i] * torch.dot(self.s[m][i, :], q))
                q -= self.alpha[m][-1] * self.y[m][i, :]
            self.alpha[m].reverse()
            r = q # recall that Hg is initialized as I
            for i in range(len(self.rho[m])): # from the oldest update to the most recent update
                beta = self.rho[m][i] * torch.dot(self.y[m][i, :], r)
                r += self.s[m][i, :] * (self.alpha[m][i] - beta)
            r = r.unsqueeze(1)
            Hv.append(r)
            self.alpha[m].clear()
    ...
    def _append_s_y(self, m, s, y):
        ...
        if (not torch.isinf(sTs)) and (yTs.item() > 10**(-4) * sTs.item() * math.sqrt(gTg)):
            if m in self.s and (len(self.s[m]) == self.num_s_y_pairs): # drop oldest
                self.s[m] = self.s[m][1:]
                self.y[m] = self.y[m][1:]
                self.rho[m] = self.rho[m][1:]

            if m not in self.s: # init
                self.s[m] = s.unsqueeze(1).t()
                self.y[m] = y.unsqueeze(1).t()
                self.rho[m] = []
            else:
                self.s[m] = torch.cat([self.s[m], s.unsqueeze(0)], dim=0) # append row
                self.y[m] = torch.cat([self.y[m], y.unsqueeze(0)], dim=0) # append row
                self.rho[m].append(1 / yTs)
    ...
    def _get_layer_update_direction(self, m, p_grad_mat, damping):
        ...
        """
        v = self._get_LBFGS_Hv(m, torch.mm(p_grad_mat, self.H_a[m]))
